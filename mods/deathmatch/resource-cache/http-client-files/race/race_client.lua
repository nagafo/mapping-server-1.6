...
addEventHandler('onClientResourceStart', g_ResRoot,
	function()
		...
		g_dxGUI = {
			--timepassed = dxText:create('Map time: 0:00:00', 5, screenHeight - 5 - dxGetFontHeight(1.1, 'default-bold')*1.8, false, 'default-bold', 1.1, 'left'),
			--mapdisplay = dxText:create('Map name: none', 5, screenHeight - 5 - dxGetFontHeight(1.1, 'default-bold')/1.5, false, 'default-bold', 1.1, 'left')
		}
...
function initRace(vehicle, checkpoints, objects, pickups, mapoptions, ranked, duration, gameoptions, mapinfo, playerInfo)
	...
	--g_dxGUI.mapdisplay:text("Map name: "..g_MapInfo.name)
	...
	-- GUI
	--g_dxGUI.timepassed:text('Map time: 0:00:00')
...
function updatePickups()
	local angle = math.fmod((getTickCount() - 1) * 360 / 2000, 360)
...
function updateTime()
	...
	if not isPlayerFinished(g_Me) then
		--g_dxGUI.timepassed:text("Map time: "..msToTimeStr(msPassed))
	end
